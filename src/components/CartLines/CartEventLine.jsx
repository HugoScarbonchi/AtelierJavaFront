...
export default function CartEventLine({detail}) {

    const [isHover, setIsHover] = useState(false)
    const [colorCross, setColorCross] = useState("#5D5A88")

    const clickOnCross = (ticketId) => {
        FeteDeLaMusique.removeTicketToCart(ticketId)
    }

    return(
        <tr className="text-primary" onMouseEnter={() => setIsHover(true)} onMouseLeave={() => setIsHover(false)}>
            ...
            <td className="w-16 py-3 text-end transition duration-300 pl-7 " style={{opacity: isHover ? 1 : 0}}>
                <svg className="svg-icon cross transition duration-300 hover:scale-110 cursor-pointer" viewBox="0 0 20 20" width="40" height="40"
                     ...
                     onClick={() => clickOnCross(detail.ticket.id)}
                >
                    <path
                        fill={colorCross}
